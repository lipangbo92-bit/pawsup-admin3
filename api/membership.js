  ...
  try {
    ...
    switch (action) {
      ...
      case 'updateMembershipLevel':
        ...
        break;
      case 'uploadLevelIcon':
        const { level, iconBase64 } = req.body;
        result = await uploadLevelIcon(level, iconBase64);
        break;
      default:
  ...
  try {
    ...
    if (existingData.length > 0) {
      ...
      for (const doc of existingData) {
        ...
        if (!doc.benefits) {
          const defaultLevel = DEFAULT_LEVELS.find(l => l.level === doc.level);
          if (defaultLevel) updateData.benefits = defaultLevel.benefits;
        }
        if (doc.iconUrl === undefined) {
          updateData.iconUrl = '';
        }
...
// 上传等级图标到云存储
async function uploadLevelIcon(level, iconBase64) {
  if (!level || !iconBase64) {
    return { success: false, error: 'Missing level or icon data' };
  }

  try {
    // 将 base64 转换为 buffer
    const buffer = Buffer.from(iconBase64.replace(/^data:image\/\w+;base64,/, ''), 'base64');
    
    // 上传到云存储
    const result = await app.uploadFile({
      cloudPath: `membership-icons/level-${level}-${Date.now()}.png`,
      fileContent: buffer
    });
